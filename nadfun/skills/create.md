...
const saltResponse = await fetch(`${CONFIG.apiUrl}/agent/salt`, {
  method: "POST",
  headers: { "Content-Type": "application/json", ...headers },
  body: JSON.stringify({ creator: account.address, name: "My Token", symbol: "MTK", metadata_uri }),
})
...
```typescript
// Get deploy fee
const feeConfig = await publicClient.readContract({ address: CONFIG.CURVE, abi: curveAbi, functionName: "feeConfig" })
...
if (initialBuyAmount > 0n) {
  minTokens = await publicClient.readContract({ address: CONFIG.LENS, abi: lensAbi, functionName: "getInitialBuyAmountOut", args: [initialBuyAmount] })
}

// Create token
const createArgs = { name: "My Token", symbol: "MTK", tokenURI: metadata_uri, amountOut: minTokens, salt: salt as `0x${string}`, actionId: 1 }

const estimatedGas = await publicClient.estimateContractGas({
  address: CONFIG.BONDING_CURVE_ROUTER, abi: bondingCurveRouterAbi, functionName: "create",
  args: [createArgs], account: account.address, value: deployFeeAmount + initialBuyAmount,
})

const hash = await walletClient.writeContract({
  address: CONFIG.BONDING_CURVE_ROUTER, abi: bondingCurveRouterAbi, functionName: "create",
  args: [createArgs], account, chain, value: deployFeeAmount + initialBuyAmount, gas: estimatedGas + estimatedGas / 10n,
})
...
**Contract Addresses**: Testnet `0x24dFf9B68fA36f8400302e2babC3e049eA19459E` | Mainnet `0x42e75B4B96d7000E7Da1e0c729Cec8d2049B9731`

```typescript
// Check claimable
const { tokens } = await fetch(`${CONFIG.apiUrl}/agent/token/created/${accountAddress}`, { headers }).then(r => r.json())

// Claim
const claimableTokens = tokens.filter(t => t.reward_info.claimable && BigInt(t.reward_info.amount) - BigInt(t.reward_info.claimed_amount) > 0n)
const hash = await walletClient.writeContract({
  address: CREATOR_TREASURY, abi: creatorTreasuryAbi, functionName: "claim",
  args: [claimableTokens.map(t => t.token_info.token_id), claimableTokens.map(t => BigInt(t.reward_info.amount) - BigInt(t.reward_info.claimed_amount)), claimableTokens.map(t => t.reward_info.proof)],
})
```
